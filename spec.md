...
### 2.2.4 투자 수익 계산기 (프론트엔드 인터랙티브)

사용자가 예상 낙찰가, 매도액, 대출 비율 등을 직접 조정하여 실수익금과 기간별 수익률을 즉시 확인할 수 있는 프론트엔드 전용 계산기입니다. 백엔드 API 호출 없이 순수 프론트엔드 계산으로 동작합니다.

#### 사용자 입력값 (편집 가능)

| 입력 항목 | 기본값 | 설명 |
|-----------|--------|------|
| 예상 낙찰가 | valuation moderate 입찰적정가 | 만원 단위 편집 |
| 예상 매도액 | valuation moderate 매도적정가 | 만원 단위 편집 |
| 대출 비율 | 0% | 슬라이더 0%~최대(아파트 70%, 그 외 60%), step 5% |
| 취득세율 | 1.1% | 소수점 입력 가능 |
| 대출 이율 | 5% | 소수점 입력 가능 |
| 이사비 | 0원 | 만원 단위 편집 |
| 관리비 | 0원 | 만원 단위 편집 |
...
매도차액 = 예상매도액 - 예상낙찰가
대출금액 = 예상낙찰가 × 대출비율%
최대대출비율 = 부동산유형이 아파트/공동주택 ? 70% : 60%

[취득비용]
  취득세     = 낙찰가 × 취득세율 (기본 1.1%)
  농어촌특별세 = 전용면적 > 85㎡ ? 낙찰가 × 0.2% : 0
  법무사비    = 80만원 (고정)
  취득비용계  = 취득세 + 농어촌특별세 + 법무사비
...
고정비용 = 취득비용계 + 명도비계 + 양도비계
```

#### 기간별 수익 분석 (0~6개월)

보유기간(N개월)에 따라 대출이자비용이 누적되므로, 실수익금과 수익률이 기간별로 달라집니다.

```
실투자금(N)   = (낙찰가 - 대출금) + N × 월이자
대출이자(N)   = N × 월이자
총비용(N)     = 고정비용 + 대출이자(N)
실수익금(N)   = 매도차액 - 총비용(N)
수익률(N)     = 실수익금(N) / 실투자금(N) × 100
```

**기간별 테이블 표시 항목** (0~6개월, 열 방향):

| 행 | 설명 | 기간별 변동 |
|----|------|-------------|
| 실투자금 | 자기자본 + 누적 대출이자 | 변동 |
| 매도차액 | 매도액 - 낙찰가 | 고정 |
| 취득비용 | 취득세 + 농특세 + 법무사비 | 고정 |
| 명도비용 | 이사비 + 관리비 + 수리비 | 고정 |
| 양도비용 | 양도세 + 중개비 | 고정 |
| 대출이자 | 월이자 × 보유개월수 | 변동 |
| **총 비용** | 취득+명도+양도+대출이자 | 변동 |
| **실수익금** | 매도차액 - 총 비용 | 변동 |
| **수익률** | 실수익금 / 실투자금 × 100 | 변동 |

- 실투자금이 0 이하인 경우 수익률은 "N/A"로 표시
- 양수 수익률/실수익금: 초록색, 음수: 빨간색
...
| 투자계산기 | `InvestmentCalculator` | 투자 수익 계산기 (인터랙티브) |
| 뉴스 | `NewsTab` | 뉴스/동향 |
...
| `InvestmentCalculator` | `bidPrice`, `salePrice`, `propertyType`, `area` | 투자 수익 계산기: 가격/대출/취득비용/명도비용/양도비용 편집 + 기간별(0~6개월) 실투자금·수익률 테이블 |
...
| 항목 | 현재 상태 | 비고 |
|------|-----------|------|
| 수선비 (repair_cost) | 투자계산기에서 사용자 편집 가능 (기본 0원) | 백엔드 valuation 노드에서는 여전히 0원 고정 |
| 양도소득세 (capital_gains_tax) | 투자계산기에서 매도차액의 6% 적용 | 간이 세율로, 실제 보유기간/다주택 기반 정밀 산출 미구현 |
| 다주택자 취득세 | 투자계산기에서 세율 편집 가능 (기본 1.1%) | `num_houses` 기반 자동 계산 미구현 |
| 소액임차인 기준 | 서울 기준 고정 (1.65억) | 지역별 기준 차등 적용 필요 |
| 보유기간별 수익률 | 투자계산기에서 0~6개월 기간별 표시 | 6개월 초과 기간 분석 미지원 |
